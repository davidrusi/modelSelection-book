...
To summarize what we saw so far, 
...
@zhou_quan:2022 proposed a locally informed thresholded (LIT) algorithm that's theoretically appealing in being scalable to high dimensions, also under suitable sparsity conditions that guarantee sufficient posterior concentration. 
...
Section \@ref(mcmc-basics) gives a brief introduction to MCMC, discussing popular algorithms like Metropolis-Hastings, Gibbs sampling and Hamiltonian Monte Carlo.
The actual adaptation of these algorithms to our model selection settings of interest is given in Chapters \@ref(glm), \@ref(gam) and \@ref(ggm) for generalized linear models, generalize additive models and Gaussian graphical models respectively. 
...
**Ergodic theorem.**
*If the Markov Chain $\bfeta^{(0)},\bfeta^{(1)},\ldots$ is irreducible, aperiodic and recurrent, then it has a unique stationary distribution given by $p(\bfeta \mid \by)$.*



Briefly, the proof requires two steps: first, one shows that the Markov chain has a unique stationary distribution, and second that the stationary distribution is the desired $p(\bfeta \mid \by)$.
Uniqueness of the stationary distribution holds when the chain is irreducible, aperiodic and not transient.
...
- Irreducibility holds as long as there's positive probability of reaching any state $\bfeta_a$ from any starting state $\bfeta_b$.

Proving that the stationary distribution is $p(\bfeta \mid \by)$ can be done by showing that, if $\bfeta^{(b)} \sim p(\bfeta \mid \by)$ then the distribution of $(\bfeta^{(b)},\bfeta^{(b+1)})$ is equal to that of $(\bfeta^{(b+1)},\bfeta^{(b)})$ (this is called reversibility), which implies that the marginal distribution of $\bfeta^{(b+1)}$ is equal to that of $\bfeta^{(b)}$ and hence that $p(\bfeta \mid \by)$ is indeed a stationary distribution.

**Gibbs sampling.**

Let $B$ be the number of desired draws from $p(\bfeta \mid \by)$ and
$\bfeta_{-j}= (\eta_1,\ldots,\eta_{j-1},\eta_{j+1},\ldots,\eta_d)$ the vector $\bfeta$ after removing its $j^{th}$ entry.

1. Initialize $\bfeta^{(0)}$, set $b=1$.

2. Set $\bfeta^{(b)}=\bfeta^{(b-1)}$. For $j=1,\ldots,d$, update $\eta_j^{(b)} \sim p(\eta_j \mid \bfeta_{-j}^{(b)},\by)$.

3. If $b=B$ stop, else set $b=b+1$ and go back to Step 2.



We finally discuss Hamiltonian Monte Carlo (HMC) as powerful alternative to Gibbs, that is applicable to situations where the parameters have a continuous distribution $p(\bfeta \mid \by)$. Note that this precludes the application of HMC to model selection, where $\bfeta=\bgamma$ is discrete and $\btheta$ follows a mixture of continuous and discrete distributions.
HMC is a MH where the proposal depends on the gradient of $\log p(\bfeta \mid \by)$. By using gradient information, the proposals can sometimes explore the posterior distribution more efficiently.
...
\nonumber
\end{align}
for some matrix $M$ (typically diagonal) %, ideally given by the inverse posterior covariance of $V(\bfeta \mid \by)$).
...
That is, one introduces $d$ momentum variables ${\bf v}$, one for each of the $d$ elements in $\bfeta$. The algorithm updates these momentum variables to a noisy version of the gradient $\nabla_\theta \log p(\bfeta \mid \by)$, and then deterministically updates $\bfeta$ in the direction of the momentum. The process is repeated $L$ steps, where the momentum is updated based on the new gradient, and $\bfeta$ updated based on the new momentum.
...
- A popular algorithm called {\it no U-turn sampler} sets $L$ automatically, and is used by the Stan and numpyro softwares.
