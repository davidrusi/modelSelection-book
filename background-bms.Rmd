...
Markov Chain Monte Carlo algorithms provide the basic workhorse for Bayesian model selection when one cannot exhaustively enumerate the set of possible models.
These algorithms have proven surprisingly effective in practice, provided that posterior model probabilities $p(\bgamma \mid \by)$ concentrate on sparse models, and that one can quickly evaluate the marginal likelihood $p(\by \mid \bgamma)$ and prior probability $p(\bgamma)$ for each model $\bgamma$.

As usual in MCMC, the main idea is to approximate the posterior distribution $p(\bgamma \mid \by)$ by obtaining a sufficiently large number of samples from it.
There are many possible MCMC algorithms.
A classical strategy in the variable selection setting where $\bgamma=(\gamma_1,\ldots,\gamma_p)$ and $\gamma_j=\mbox{I}(\beta_j \neq 0)$ are variable indicators is to use Gibbs sampling (@madigan:1994, @hoeting:1999), where one updates a single variable indicator $\gamma_j$ at a time.
These are sometimes referred to as birth-death moves.
...
We describe these algorithms in further detail in Chapter \@ref(glm). Briefly, the algorithms implemented in `modelSelection` are:

- Function `modelSelection` uses an MCMC model search, based on classical Gibbs sampling. Functions such as `bestBIC` implementing model search for $L_0$ criteria also use Gibbs sampling.

- Function `modelSelectionGGM` also implements the birth-death-swap  [@yangyun:2016] and locally informed thresholded (LIT) algorithms [@zhou_quan:2022]. 
