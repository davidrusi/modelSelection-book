...
and serve as documentation for the R package `modelSelection` (previously called `mombf`) which I have been developing over the years.
With these two goals in mind, the book evolved into a hands-on guide to model selection.

The book focuses on sparse inference, mainly Bayesian model selection (BMS) and averaging (BMA), for a number of popular models listed below. It also implements L0 criteria like the AIC, BIC or EBIC (as well as more general information criteria).
The R package's C++ implementation is not optimal, but it's designed to be minimally scalable in sparse high-dimensional settings (large $p$).
...
For a quick start guide with `modelSelection`, see Section \@ref(quick-start). The main models handled by the package are:
